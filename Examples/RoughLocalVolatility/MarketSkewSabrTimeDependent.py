...
for i in range(0, 16):
    ...
    # skew lv
    lv_output = LocalVolFunctionals.derivatives_local_vol_log_normal_sabr(delta_time[i], np.array(strikes), f0, alpha_i, rho_i, nu_i)
...
# plt.plot(delta_time, ratio_fit, label="%s+ %s t + %s t^2" % (round(popt[0], 5), round(popt[1], 5), round(popt[2], 5)), color="black",
