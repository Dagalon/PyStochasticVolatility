...
sigma = 0.3
beta = 0.4
local_vol_mc = partial(LocalVolFunctionals.log_cev_diffusion(beta=beta, sigma=sigma))
...
# expansion info
lv_f0 = LocalVolFunctionals.cev_diffusion(T)
...
for i in range(0, no_dt_s):
    # expansion info
    lv_f0 = LocalVolFunctionals.cev_diffusion(dt[i], np.asfortranarray(f0), beta, sigma)[0]
    fd_lv_f0 = LocalVolFunctionals.first_derive_cev_diffusion(dt[i], np.asfortranarray(f0), beta, sigma)[0]
    sd_lv_f0 = LocalVolFunctionals.second_derive_cev_diffusion(dt[i], np.asfortranarray(f0), beta, sigma)[0]
