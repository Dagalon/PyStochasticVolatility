...
def get_path_multi_step(tis, x0, y0, ft: ql.ForwardCurve, k, no_paths, eta_vol, rnd_generator) -> map:
    ...
    for j in range(1, no_time_steps):
        ...
        int_t_delta = (int_t + CheyetteTools.gamma(0.0, delta_t, k) * x_paths[:, j - 1] + CheyetteTools.int_lambda(
            delta_t, k) * y_paths[:, j - 1] +
                       std_b_t * (np.multiply(rho, zs[:, 0]) + np.multiply(np.sqrt(1.0 - rho * rho),
                                                                           zs[:, 1]))) + int_t_0
...
def get_path_multi_step_forward_measure(tis, x0, y0, ft: ql.ForwardCurve, k, no_paths, eta_vol, rnd_generator) -> map:
    ...
    for j in range(1, no_time_steps):
        ...
        v_t = CheyetteTools.variance(tis[j - 1], tis[j], k, eta_i)
        y_paths[:, j] = beta_t * beta_t * y_paths[:, j - 1] + v_t
        drift_forward_measure = - gamma_drift_t * np.power(eta_i, 2.0)
        x_paths[:, j] = beta_t * x_paths[:, j - 1] + gamma_t * y_paths[:, j - 1] + np.multiply(np.sqrt(v_t), zs[:, 0]) + drift_forward_measure
