...
def get_quadratic_option_lv_normal_sabr_watanabe_expansion(f0, k, t, alpha, nu, rho):
    ...
    # T
    # g2
    # b_t = 0.25 * np.power(nu * rho, 2.0) * (np.power(y, 3.0) * phi_y + y * phi_y + cphi_y_inv)
    b_t = 0.25 * np.power(nu * rho, 2.0) * y * y * phi_y

    # g_3
    c_t = (np.power(nu * rho_inv, 2.0) * (4.0 * y*phi_y + 3.0 * cphi_y_inv) + 2.0 * np.power(nu * rho, 2.0) * (y * phi_y)) / 6.0

    # T^{3/2}
    # g_2 * g_3
    d_1_t = np.power(nu, 3.0) *  np.power(rho_inv, 2.0) * rho * (2.0 * np.power(y, 4.0) + 3.0 * y * y + 3.0 * y) * phi_y
    d_2_t = np.power(nu * rho, 3.0) * (np.power(y,4.0) + 3) * phi_y
    ...
    # g_4
    f_1_t = np.power(nu * rho, 3.0) * (np.power(y, 2.0) - 3.0) * phi_y / 144.0
    # f_2_t = np.power(nu, 3.0) * np.power(rho_inv, 2.0) * rho *

    return alpha * alpha * t * (
            g_y + a_t * np.sqrt(t) + (b_t + c_t) * t + (e_t + d_t + f_1_t) * np.power(t, 1.5))
...
def get_quadratic_option_normal_sabr_watanabe_expansion(f0, k, t, alpha, nu, rho):
    ...
    # T
    # g_3
    b_1_t = np.power(nu * rho, 2.0) * y * phi_y / 3.0

    # g^2_2
    b_21_t = 0.25 * np.power(nu * rho, 2.0) * (np.power(y, 3.0) * phi_y + y * phi_y + cphi_y_inv)
    ...
    return alpha * alpha * t * (
            g_y + a_t * np.sqrt(t) + (b_1_t + b_21_t + b_22_t) * t + (c_1_t + c_21_t + c_22_t + c_3_t) * np.power(t,
                                                                                                                  1.5))
...
def get_option_normal_sabr_loc_vol_expansion(f0, k, t, alpha, nu, rho, option_type):
    ...
    e_4_2_t = np.power(nu, 3.0) * np.power(rho_inv, 2.0) * rho *  (3.0 * np.power(y, 3.0) * phi_y + 2.0 * y * phi_y + cphi_y_inv) / 96.0

    e_4_2_t = 0.0
    call_price = alpha * np.sqrt(t) * (g_y + phi_y * np.sqrt(t) * a_t + phi_y * t * (b_t + c_t) +
                                       phi_y * np.power(t, 1.5) * (e_23_t + e_22_t + e_4_1_t) + np.power(t, 1.5) * e_4_2_t)
