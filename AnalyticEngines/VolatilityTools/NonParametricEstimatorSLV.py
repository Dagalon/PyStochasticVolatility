...
# @nb.jit("f8[:](f8[:],f8[:],f8[:],f8)", nopython=True, nogil=True)
def gaussian_kernel_estimator_skew_slv(v_t: Types.ndarray, x_t: Types.ndarray, x: Types.ndarray, h: float):
    no_x = len(x)
    estimator = np.zeros(no_x)

    for i in range(0, no_x):
        diff_i = x_t - x[i]
        k_x_i = gaussian_kernel(diff_i, h)
        marginal = np.sum(k_x_i)
        sigma_t_i = AnalyticTools.scalar_product(v_t, k_x_i) / marginal
        estimator[i] = (sigma_t_i * AnalyticTools.scalar_product(diff_i / (h * h), k_x_i) -
                        AnalyticTools.sum_product(diff_i / (h * h), k_x_i, v_t)) / marginal
